...
- **Capas — detalles de render (`renderComposicion`):** imagen con `fit` (`cover`/`contain` = "Cubrir/Contener", permite achicar) + `zoom` (50–300%, `transform:scale` anclado al `foco`) + `foco` + `oscurecer`; logo con `tamano` (=**Alto**, px) + `maxw` (=**Ancho máx**, %). Textos por defecto **vacíos** (los inputs muestran placeholder; ya NO se renderiza lorem "Tu titular aquí"/"Texto de apoyo breve" como contenido real).
- **Override "vacío = hereda":** `composicionEfectiva` IGNORA overrides cuyo valor sea `""` (string vacío) → así un campo vaciado por tamaño hereda el global. Para quitar algo en un tamaño concreto se usa el **ojo** (`visible:false`), no vaciar el campo. Esto **sana** el bug del máster que quedaba con override `imagen.url:""` y perdía la foto. `tieneOverride` también ignora `""`.
- **Editor por tamaño (`renderFormTamaño`, pestaña "Editar bloque"):** ahora incluye **Fondo de color** (con swatches `swatchesFondoOv(fmt)`), Imagen (URL que **hereda** si se deja vacía + Ajuste/Zoom/Foco/Oscurecer) y Logo (Alto + Ancho máx).
