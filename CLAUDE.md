...
- **Asistente IA**: 3er producto "Google Search Ads" (`iaSetFormato('ads')`; oculta tipo-email y fotos `#ia-fotos-wrap`; `abrirIA()` preselecciona según sección). `generarConIA` rama ads (sin imágenes) → **`insertarAdsIA`**: guarda la campaña ÍNTEGRA en **`p.adsData`** (fuente de verdad del CSV) y la pinta como bloques legibles (tabla de keywords con `[exacta]`/`"frase"`, tablas de titulares/descripciones con contador de caracteres, negativas, ruta visible).
- **Export**: `abrirExportar` detecta formato ads → modal con **CSV de keywords+negativas** (`descargarCSVKeywords`: cabecera `Campaign,Ad Group,Keyword,Criterion Type`, Exact/Phrase, negativas como Negative Phrase, las de campaña con Ad Group vacío) y **CSV de anuncios RSA** (`descargarCSVAnuncios`: Headline 1..N, Description 1..4, Path 1/2, Final URL=`adsData.urlFinal`) + resumen HTML (`descargarPieza`). Cabeceras en inglés = las que Google Ads Editor reconoce solo. BOM UTF-8 (`_descargarCSV`).
