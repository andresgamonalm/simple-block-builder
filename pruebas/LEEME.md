...
| `nucleo` | **Sin navegador.** Modelo de campaña (`nucleo/modelo.js`) e importador/exportador de Ads Editor (`nucleo/ads-editor.js`). Prueba de oro: importar `pruebas/datos/ads-editor-referencia.csv` (copia anonimizada del CSV real del usuario) y exportarlo da el mismo archivo byte a byte. Con `MP_CSV_REAL=/ruta.csv` corre también contra el real. |
| `nucleo-contrato` | **Sin navegador.** `CONTRATO-CAMPANA.md` coincide con lo que genera el esquema (`node nucleo/generar-contrato.js`). |
| `reglas` | **Sin navegador.** Motor de reglas (`nucleo/reglas.js`, reglas G·P·C·K, P = protocolo de la casa). Taxonomía de nombres, UTM y nombre del archivo exportado. Sobre el CSV de referencia encuentra exactamente lo de la auditoría (4 errores, incluida la negativa amplia «comprar auto» que bloquea «comprar seguro de auto»); cada regla tiene su caso; una campaña sana da cero hallazgos. |
| `versiones` | **Sin navegador.** `nucleo/versiones.js`: estados y transiciones, huella de contenido, nombres fijos tras publicar, diferencias por `id` y el **archivo de solo cambios** para Ads Editor (nuevo · modificado · reemplazado = anterior con `Status Removed` + nuevo · eliminado), sobre la campaña de referencia. |
| `iniciativas` | **Sin navegador.** `functions/api/iniciativas.js` con una D1 real (SQLite de Node, misma interfaz que Cloudflare) y cookie de sesión firmada: versiones, 409 ante conflicto, espacios aislados, aprobar exige 0 errores (validado en el servidor), nombres fijos tras publicar, historial y poda (50 + exportada + publicada). |
...
| `diagnostico` | **Sin navegador.** "Tu campaña actual": lectura de los informes de Google Ads (español/inglés, UTF-8/UTF-16, montos en CLP, líneas de título), cifras calculadas por el servidor, diagnóstico con Gemini simulado, negativas que no bloquean lo que convierte ni el objetivo, y la generación que recibe el aprendizaje sin perder el objetivo. Datos inventados en `pruebas/datos/`. |
| `campana-actual` | "Tu campaña actual" en el editor de Search: adjuntar CSV, CSV UTF-16, Excel y capturas (archivo y Ctrl+V), analizar, sumar negativas, XLSX de cambios para Ads Editor, rearmar con el aprendizaje, volver a la versión anterior y el asistente con el aprendizaje. |
